    ...
    if (id <= 0 || data == nullptr || data->empty()) {
        ...
        if (idExists) {
            ...
            if (head == nullptr || id < head->data.id) {
                // Insert the new node as git the new head
                newNode->next = head;
...
bool LinkedList::deleteNode(int id) {
    ...
    while (current) {
        if (current->data.id == id) {
            // Found the node, adjust the pointers to delete it
            if (current->prev) {
            ...
            deleted = true; // Node successfully deleted
            }
        current = current->next;
    }

    return deleted; // Return whether node was deleted or not
}
...
bool LinkedList::exists(int id) {
	/*****************************************
 	Checks whether a node with the specified ID exists in the linked list.
	 
 	@param id The ID of the node to search for.
 	@return true if a node with the specified ID exists in the list, false otherwise.
    *****************************************/
